...
void testClosures() {
    auto tests = [
        VMTestCase!int(`let newClosure = fn(a) {fn() { a; };};let closure = newClosure(99);closure();`, 99),
        VMTestCase!int(`let newAdder = fn(a, b) {fn(c) { a + b + c };};let adder = newAdder(1, 2);adder(8);`, 11),
        VMTestCase!int(
        ...
        VMTestCase!int(
            `let newAdderOuter = fn(a, b) {let c = a + b;fn(d) {let e = d + c;
            fn(f) { e + f; };};};let newAdderInner = newAdderOuter(1, 2)
        ...
        VMTestCase!int(
            ...
            fn() { one() + two(); };};let closure = newClosure(9, 90);closure();`, 
            99
        )
    ];  
